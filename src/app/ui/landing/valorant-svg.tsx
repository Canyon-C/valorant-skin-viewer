...
export const ValorantSVG = () => {
    ...
    const [translate, animateTranslate] = useAnimate();
    const [appear, animateAppear] = useAnimate();
    const [skinViewerButton, animateSkinViewerButton] = useAnimate();

    useEffect(() => {
        const animate_svg1 = async () => {
            await animate1(svg1.current, {pathLength: 1}, {duration: 1.5});
        }
        const animate_svg2 = async () => {
            // await animate2(svg2.current, {pathLength: 0},)
            await animate2(svg2.current, {pathLength: 1}, {duration: 1.5});
            await animateTranslation();
            await featuresAppear();
            
        }
        const animateTranslation = async () => {
            await animateTranslate(translate.current, {y: "calc(-45vh)"});
        }
        const featuresAppear = async () => {
            await animateAppear(appear.current, {opacity: 1, y: "2%"});
        }
        ...
        const buttonAnimate = async () => {
            animateSkinViewerButton(skinViewerButton.current, {opacity: 1, y: "2%"});
        }
        svgAnimate();
    }, [])

    const containerVariants = {
        positioning: {y: "-50%", x: "-50%"},
    }
