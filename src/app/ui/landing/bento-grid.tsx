...
export const BentoGridItem = ({
  ...
  return (
    <div
      className={cn(
        "row-span-1 rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-4 border-2 borderAccent justify-between flex flex-col space-y-4 relative overflow-hidden",
        className
      )}
    >
      <div className="absolute inset-0 opacity-0 group-hover/bento:opacity-100 transition-opacity duration-200 pointer-events-none z-0">
        <div className="flood-border-animation w-full h-full"></div>
      </div>
...
const Skeleton = () => (
  <div className="flex flex-1 w-full h-full min-h-[6rem] rounded-xl bg-background border border-transparent"></div>
);
const items = [
  {
